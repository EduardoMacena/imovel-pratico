...
$ServiceXml = Join-Path $ServiceDir "ImovelPraticoWorkerRegistro.xml"

Copy-Item $WinSwExe $ServiceExe -Force
Copy-Item (Join-Path $AppDir "scripts\windows\templates\ImovelPraticoWorkerRegistro.xml") $ServiceXml -Force
Copy-Item (Join-Path $AppDir "scripts\windows\run-worker-registro.ps1") (Join-Path $ServiceDir "run-worker-registro.ps1") -Force

$EnvFile = Join-Path $InstallDir ".env"

if (!(Test-Path $EnvFile)) {
  Copy-Item (Join-Path $AppDir "scripts\windows\env\worker-registro.env.example") $EnvFile
...
pnpm --filter "@imovel-pratico/worker-registro" exec playwright install chromium
...
Write-Output "Start-Service ImovelPraticoWorkerRegistro"
