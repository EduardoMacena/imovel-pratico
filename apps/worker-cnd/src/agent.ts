...
function normalizarRegistros(registros: CndAgentJob["registros"]) {
	return registros
		.map((registro) => ({
			...
			complemento: registro.complemento ? String(registro.complemento).trim() : null,
		}))
...
async function processarRegistroCnd(params: {
	...
	try {
		...
		console.log("[agent-cnd] Consultando contato pela API Gateway");

		const contato = await client.buscarContatoPorCpf({
			cpf: proprietario.cpf,
		});

		console.log(`[agent-cnd] Contato retornado: ${contato.fonte}`);
		...
		console.log(`[agent-cnd] Índice ${registro.indiceCadastral} concluído com sucesso`);
	} catch (error) {
...
async function processarCndJob(client: AgentApiClient, job: CndAgentJob) {
	...
	console.log(`[agent-cnd] Processando tarefa ${job.id} com ${total} registro(s)`);
	...
	while (!shuttingDown) {
		try {
			...
			if (claimed.job.tipo !== "CND") {
				console.warn(`[agent-cnd] Job ignorado por tipo inesperado: ${claimed.job.tipo}`);
			...
			try {
				...
				console.error(`[agent-cnd] Erro ao processar job ${claimed.job.id}:`, formatted);
