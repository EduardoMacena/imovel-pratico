import { Prisma, prisma, type BuscaPreviaStatus } from "@imovel-pratico/database";
import {
...
async function recalcularPrevia(clienteId: string, previaId: string) {
  ...
  if (
    !["PRONTA", "AGUARDANDO_AUTORIZACAO_EXCEDENTE"].includes(previa.status)
  ) {
...
export async function confirmarPreviaECriarTarefa(
  ...
  try {
    ...
    await prisma.buscaPrevia.updateMany({
      ...
      data: {
        status: "AGUARDANDO_AUTORIZACAO_EXCEDENTE",
        erro: error instanceof Error ? error.message : "Erro ao autorizar prévia",
      },
    });
