...
type MonitoramentoFiltros = {
  ...
  servico?: "API_GATEWAY" | "WORKER_REGISTRO" | "WORKER_CND" | "QUEUE" | "REALTIME";
  tipo?: string;
  dataInicio?: Date;
  dataFim?: Date;
  take?: number;
...
export async function buscarResumoMonitoramentoAdmin(filtros: {
  ...
  const workersOffline = workers.filter(worker => worker.status !== "ONLINE").length;
  const workersOnline = workers.filter(worker => worker.status === "ONLINE").length;

  const statusGeral =
    eventosErro24h > 0 || workersOffline > 0 || tarefasComErroHoje > 0
      ? "ATENCAO"
      : "OPERACIONAL";
