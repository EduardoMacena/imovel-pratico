...
export default function MonitoramentoPage() {
  ...
  const [resumo, setResumo] =
    useState<MonitoramentoResumoResponse["resumo"] | null>(null);
  const [eventos, setEventos] =
    useState<ListarEventosMonitoramentoResponse["eventos"]>([]);
  const [filas, setFilas] =
    useState<ListarFilasMonitoramentoResponse["itens"]>([]);
  ...
  async function carregarMonitoramento() {
    try {
      ...
      const [clientesData, resumoData, eventosData, filasData] =
        await Promise.all([
          clientes.length > 0 ? Promise.resolve({ clientes }) : listarClientes(),
  ...
  const metricas = useMemo(() => {
    return {
      ...
      tarefasEmAndamento:
        (resumo?.tarefas.pendentes ?? 0) +
        (resumo?.tarefas.processando ?? 0),
    ...
    <>
      ...
      <PageShell>
        <PageContainer>
          <HeroGrid>
            ...
            <HeroPanel>
              <div>
                ...
                <HeroPanelTitle>
                  {formatNumber(metricas.workersOnline)} online
                </HeroPanelTitle>

                <HeroPanelSubtitle>
                  {formatNumber(metricas.workersOffline)} worker(s) offline ou
                  com atenção.
                </HeroPanelSubtitle>
              </div>
          ...
          <MetricGrid>
            <MetricCard>
              <MetricLabel>Workers online</MetricLabel>
              <MetricValue>{formatNumber(metricas.workersOnline)}</MetricValue>
              <MetricHint>Sinais recebidos nos últimos minutos.</MetricHint>
            </MetricCard>

            <MetricCard>
              <MetricLabel>Workers com atenção</MetricLabel>
              <MetricValue>{formatNumber(metricas.workersOffline)}</MetricValue>
              <MetricHint>Offline, sem heartbeat ou com erro.</MetricHint>
            </MetricCard>
                          ...
                          <div>
                            ...
                            <EventMeta>
                              ...
                              {evento.tarefa && (
                                <span>
                                  Tarefa:{" "}
                                  <TaskLink href={`/tarefas/${evento.tarefa.id}`}>
                                    abrir
                                  </TaskLink>
                                </span>
                              )}
              ...
              <PanelCard>
                <PanelHeader>
                  <div>
                    ...
                    <PanelSubtitle>
                      Heartbeat dos workers dedicados por cliente.
                    </PanelSubtitle>
                  </div>
                </PanelHeader>

                {!resumo || resumo.workers.itens.length === 0 ? (
                  <EmptyState>
                  ...
                  <WorkerList>
                    {resumo.workers.itens.map(worker => (
                      <WorkerItem key={worker.id}>
                        ...
                        {worker.fila && <ItemMuted>Fila: {worker.fila}</ItemMuted>}
